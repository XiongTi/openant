...
- ⏰ **定时任务**：内置早晨简报、AI 前沿简报、GitHub 备份、语音/图片清理等自动化任务
...
| `OPENCLAW_AGENTS` | 多 Agent 配置（JSON 格式） | - |
| `TELEGRAM_BOT_TOKEN` | Telegram Bot Token（支持多账号：`name1:token1,name2:token2`） | - |
| `TELEGRAM_BINDINGS` | Telegram 绑定 Agent（格式：`botName:agentId`） | - |
| `DINGTALK_CLIENT_ID` | 钉钉 Client ID（支持多账号：`name1:appkey1,name2:appkey2`） | - |
| `DINGTALK_CLIENT_SECRET` | 钉钉 Client Secret（格式需与 CLIENT_ID 对应） | - |
| `DINGTALK_BINDINGS` | 钉钉绑定 Agent（格式：`botName:agentId`） | - |
| `TAVILY_API_KEY` | Tavily 搜索 API Key | - |
| `GMAIL_APP_TOKEN` | Gmail SMTP 发送密码 | - |
| `QQMAIL_TOKEN` | QQ 邮箱 SMTP 密码 | - |
...
通过 `OPENCLAW_AGENTS` 环境变量配置多个 Agent，每个 Agent 有独立的工作空间：

```bash
OPENCLAW_AGENTS=[{"id":"agent1","name":"Agent1","workspace":"workspace-agent1"},{"id":"agent2","name":"Agent2","workspace":"workspace-agent2"}]
```

格式说明：
- `id` - Agent 唯一标识
- `name` - Agent 显示名称
- `workspace` - 工作空间目录（相对于 `/home/node/.openclaw/`）

配合渠道绑定，可以实现：
- Telegram Bot A → Agent A
- Telegram Bot B → Agent B
- DingTalk Bot → Agent A
